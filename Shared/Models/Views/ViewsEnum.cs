...
namespace Shared.Models.Views
{
    public class MWOViewsEnum
    {
        ...
        public static MWOViewsEnum GetType(string type) => List.Exists(x => x.Name == type) ? List.FirstOrDefault(x => x.Name == type)! : MWOViewsEnum.Table;
        public static MWOViewsEnum GetType(int id) => List.Exists(x => x.Id == id) ? List.FirstOrDefault(x => x.Id == id)! : MWOViewsEnum.Table;
    }
}
