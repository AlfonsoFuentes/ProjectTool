...
{
    public class NewSupplierUpdateRequestValidator : AbstractValidator<NewSupplierUpdateRequest>
    {
        private INewSupplierValidatorService _Service;
        public NewSupplierUpdateRequestValidator(INewSupplierValidatorService service)
        {
            _Service = service;
        ...
        async Task<bool> ReviewIfNameExist(NewSupplierUpdateRequest supplier, string name, CancellationToken cancellationToken)
        {
            var result = await _Service.ReviewIfNameExist(supplier.SupplierId, name);
            return !result;
        }
        async Task<bool> ReviewIfVendorCodeExist(NewSupplierUpdateRequest supplier, string vendorcode, CancellationToken cancellationToken)
        {
            var result = await _Service.ReviewIfVendorCodeExist(supplier.SupplierId, vendorcode);
            return !result;
        }
        async Task<bool> ReviewIfEmailExist(NewSupplierUpdateRequest supplier, string? email, CancellationToken cancellationToken)
        {

            var result = await _Service.ReviewIfEmailExist(supplier.SupplierId, email);
            return !result;
        }
    }
}
