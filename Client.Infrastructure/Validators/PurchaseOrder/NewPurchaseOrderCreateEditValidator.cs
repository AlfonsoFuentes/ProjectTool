...
{
    public class NewPurchaseOrderCreateEditValidator : AbstractValidator<NewPurchaseOrderCreateEditRequest>
    {
        private INewPurchaseOrderValidator PurchaseOrderValidator { get; set; }
        public NewPurchaseOrderCreateEditValidator(INewPurchaseOrderValidator purchaseOrderValidator)
        {
        ...
        async Task<bool> ReviewNameExist(NewPurchaseOrderCreateEditRequest Purchaseorder, string name, CancellationToken cancellationToken)
        {

            var result = await PurchaseOrderValidator.ValidateNameExistInPurchaseOrder(Purchaseorder.PurchaseOrderId, name);
            return !result;
        }
        async Task<bool> ReviewPRExist(NewPurchaseOrderCreateEditRequest Purchaseorder, string pr, CancellationToken cancellationToken)
        {
